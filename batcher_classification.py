...
class ClaBatcher(object):
    def __init__(self, hps, vocab):
        ...
        self.train_queue = self.fill_example_queue(#"./train/*")
            hps.train_data_path, start_file_number=0, end_file_number=65)

        self.valid_queue = self.fill_example_queue(#"./valid/*")
            hps.valid_data_path, start_file_number=0, end_file_number=20)
    ...
    def create_batches(self, mode="train", shuffleis=True):
        ...
        print ("original %s data has %d examples."%(mode, len(all_batch)*self._hps.batch_size))
        return all_batch
    ...
    def fill_example_queue(self, data_path, start_file_number=None, end_file_number=None):
        ...
        if start_file_number != None:
            filelist = filelist[start_file_number:end_file_number]
        for f in filelist:
...
class AttenBatcher(object):
    ...
    def create_batches(self, mode="train", shuffleis=True):
        ...
        for i in range(0, num_batches):
            ...
            all_batch.append(Batch(batch, self._hps, self._vocab))
        print ("with confidence 0.8 %s set has %d examples" %(mode, len(all_batch)*self._hps.batch_size))
        return all_batch
    ...
    def fill_example_queue(self, data_path):
        ...
        for f in filelist:
            ...
            while True:
                ...
                dict_example = json.loads(string_)
                review = dict_example["review"]
                conf = dict_example["confidence"]
                if float(conf) < 0.8:
                    continue
                if review.strip() =="" or len(review.split()) > 20:
                    continue
                score = dict_example["score"]
                example = Example(review, score, self._vocab, self._hps)
                new_queue.append(example)
